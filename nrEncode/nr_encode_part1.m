...
function [N, F, I, input_IL, sb_IL, IL, IB] = nr_encode_part1(E, K, crc_length, link_mode)
% -----------------------------------------------------------------------------------------------------------
% generate N
% -----------------------------------------------------------------------------------------------------------
if link_mode == 1
...
% -----------------------------------------------------------------------------------------------------------
if link_mode == 1
    if (K < (12+crc_length) || K > (140+crc_length))
        error('Unsupported K length, K should be configured in [12,140]');
    end
    if (crc_length ~= 24)
        error('Unsupported CRC length, DL only supports CRC-24.');
    end
    IL = 1; IB = 0;
else
    if (K < (12+crc_length))
        error('Unsupported K length, K should exceed 12');
    end
    if (crc_length ~= 11 && crc_length ~= 6)
        error('Unsupported CRC length, UL only supports CRC-6 and CRC-11.');
    end
    IL = 0; IB = 1;
end
